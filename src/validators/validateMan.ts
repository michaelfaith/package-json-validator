...
export const validateMan = (obj: unknown): Result => {
  ...
  if (typeof obj === 'string') {
    if (obj.trim() === '') {
      result.addIssue('the value is empty, but should be the path to a man file');
    } else if (!isManStringValid(obj)) {
      result.addIssue('the value is not valid; it should be the path to a man file');
    }
    ...
    for (let i = 0; i < obj.length; i++) {
      ...
      if (typeof item !== 'string') {
        ...
        childResult.addIssue(`item at index ${i} is empty, but should be the path to a man file`);
