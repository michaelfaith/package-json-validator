...
describe(validateGypfile, () => {
  it.each([true, false])("should return no issues for boolean values '%s'", (type) => {
  ...
  it('should return an issue if type is not a boolean (number)', () => {
    ...
    expect(result.errorMessages).toEqual(['the value should be a `boolean`, not `number`']);
  });

  it('should return error if type is not a boolean (object)', () => {
    const result = validateGypfile({});

    expect(result.errorMessages).toEqual(['the value should be a `boolean`, not `object`']);
  });

  it('should return error if type is not a boolean (Array)', () => {
    const result = validateGypfile([]);

    expect(result.errorMessages).toEqual(['the value should be a `boolean`, not `Array`']);
  });

  it('should return error if type is not a boolean (string)', () => {
    const result = validateGypfile('the fragile');

    expect(result.errorMessages).toEqual(['the value should be a `boolean`, not `string`']);
  });
  ...
  it('should return error if type is not a boolean (undefined)', () => {
    ...
    expect(result.errorMessages).toEqual(['the value should be a `boolean`, not `undefined`']);
  });

  it('should return error if type is not a boolean (null)', () => {
    const result = validateGypfile(null);

    expect(result.errorMessages).toEqual(['the value is `null`, but should be a `boolean`']);
  });
});
