...
describe(validateName, () => {
  ...
  it('should return an issue if the value is an empty string', () => {
    const result = validateName('');
    expect(result.errorMessages).toEqual(['the value is empty, but should be a valid name']);
    expect(result.issues).toHaveLength(1);
  });
  ...
  ])('should return an issue if the value is an invalid name (%s)', (input, expected) => {
