...
describe(validatePrivate, () => {
  ...
  it('should return an issue if type is not a boolean (number)', () => {
    ...
    expect(result.errorMessages).toEqual(['the type should be a `boolean`, not `number`']);
  });

  it('should return error if type is not a boolean (object)', () => {
    const result = validatePrivate({});

    expect(result.errorMessages).toEqual(['the type should be a `boolean`, not `object`']);
  });

  it('should return error if type is not a boolean (Array)', () => {
    const result = validatePrivate([]);

    expect(result.errorMessages).toEqual(['the type should be a `boolean`, not `Array`']);
  });

  it('should return error if type is not a boolean (string)', () => {
    const result = validatePrivate('the fragile');

    expect(result.errorMessages).toEqual(['the type should be a `boolean`, not `string`']);
  });
  ...
  it('should return error if type is not a boolean (undefined)', () => {
    ...
    expect(result.errorMessages).toEqual(['the type should be a `boolean`, not `undefined`']);
  });
